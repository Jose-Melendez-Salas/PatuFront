...
const BitacoraFicha = ({ asistencia, notas, acuerdos, compromisos, color }) => (
    ...
    >
        ...
        <p className="text-sm break-words"><span className="font-semibold">Notas:</span> {notas || "Sin notas"}</p>
        <p className="text-sm break-words"><span className="font-semibold">Acuerdos:</span> {acuerdos || "—"}</p>
        <p className="text-sm break-words"><span className="font-semibold">Compromisos:</span> {compromisos || "—"}</p>
    </div>
);
...
const FichaAlumno = () => {
    ...
    const [materiaSeleccionadaParaAsignar, setMateriaSeleccionadaParaAsignar] = useState(null);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                // 4) Materias (todas)
                const resMaterias = await fetch(`https://apis-patu.onrender.com/api/materias/`, { headers });
                if (resMaterias.ok) {
                ...
                const resCursadas = await fetch(`https://apis-patu.onrender.com/api/cursada/alumno/${alumno.id_usuario}`, { headers });
                if (resCursadas.ok) {
                ...
                const resCalif = await fetch(`https://apis-patu.onrender.com/api/calificacion/alumno/${alumno.id_usuario}`, { headers });
                if (resCalif.ok) {
    ...
    const fetchDisponibilidadesPorMatricula = async (matriculaAlumno, accessToken) => {
        ...
        try {
            ...
            const res = await fetch(`https://apis-patu.onrender.com/api/disponibilidades/alumno/${matriculaAlumno}`, { headers });
    ...
    const fetchDisponibilidadesPorDia = async (matriculaAlumno, diaUI) => {
        ...
        try {
            ...
            const dia = normalizar(diaUI);
            const res = await fetch(`https://apis-patu.onrender.com/api/disponibilidades/alumno/${matriculaAlumno}/dia/${encodeURIComponent(dia)}`, { headers });
            if (!res.ok) return;
            const { data = [] } = await res.json();
    ...
    const handleCrearMateria = async (e) => {
        ...
        if (!nClave || !nNombre || !nGrupo) {
            Swal.fire('Campos incompletos', 'Completa clave, nombre y grupo.', 'warning');
            return;
        }
        ...
        try {
            ...
            // refrescar materias
            const resMat = await fetch(`https://apis-patu.onrender.com/api/materias/`, { headers });
            const matJson = await resMat.json();
            setMaterias(matJson.data || []);
            // limpiar formulario
            setNClave(''); setNNombre(''); setNGrupo('');
    ...
    const handleAsignarMateria = async (e) => {
        ...
        try {
            ...
            // refrescar cursadas
            const resCurs = await fetch(`https://apis-patu.onrender.com/api/cursada/alumno/${alumnoData.id_usuario}`, { headers });
            if (resCurs.ok) {
            ...
            console.error(err);
            Swal.fire('Error', err.message || 'No se pudo asignar la materia.', 'error');
        }
    };
...
const handleBorrarMateria = async (idMateria) => {
    ...
    try {
        ...
        const notasDeEstaMateria = calificaciones.filter(c => Number(c.id_materia) === Number(idMateria));
        ...
        if (notasDeEstaMateria.length > 0) {
            ...
            const promesasDeBorrado = notasDeEstaMateria.map(nota => {
                return fetch(`https://apis-patu.onrender.com/api/calificacion/${idAlumno}/${idMateria}/${nota.unidad}`, {
                    method: 'DELETE',
                    headers
                });
            });
        ...
        const res = await fetch(`https://apis-patu.onrender.com/api/cursada/${idAlumno}/${idMateria}`, {
            method: 'DELETE',
            headers
        });
        ...
        setCursadas(prev => prev.filter(c => Number(c.id_materia) !== Number(idMateria)));
        setCalificaciones(prev => prev.filter(c => Number(c.id_materia) !== Number(idMateria)));
...
const handleActualizarCalificacion = async (e) => {
    ...
    if (!materiaAEditar || !unidadAEditar || calificacionAEditar === "") {
        Swal.fire('Campos incompletos', 'Indica unidad y calificación', 'warning');
        return;
    }
    ...
    try {
        ...
        const res = await fetch(`https://apis-patu.onrender.com/api/calificacion/${idAlumno}/${materiaAEditar}/${unidadAEditar}`, {
        ...
        Swal.fire('Actualizado', 'La calificación ha sido modificada.', 'success');

        // Refrescar la lista de calificaciones para que se vea el cambio en la gráfica
        const resCal = await fetch(`https://apis-patu.onrender.com/api/calificacion/alumno/${idAlumno}`, { headers });
        if (resCal.ok) {
    ...
    const handleAsignarCalificacion = async (e) => {
        ...
        if (isNaN(valNum) || valNum < 0 || valNum > 100) {
            Swal.fire('Valor inválido', 'La calificación debe ser un número entre 0 y 100.', 'warning');
            return;
        }
        ...
        try {
            ...
            const res = await fetch(`https://apis-patu.onrender.com/api/calificacion/`, {
                method: 'POST', headers, body: JSON.stringify(body)
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(data.message || 'Error al asignar calificación.');
            Swal.fire('¡Guardada!', 'Calificación registrada.', 'success');
            // refrescar calificaciones
            const resCal = await fetch(`https://apis-patu.onrender.com/api/calificacion/alumno/${alumnoData.id_usuario}`, { headers });
            if (resCal.ok) {
                const calJson = await resCal.json();
                setCalificaciones(calJson.data || []);
            }
            ...
            console.error(err);
            Swal.fire('Error', err.message || 'No se pudo guardar la calificación.', 'error');
        }
    };
            ...
            <main className="p-4 animate-fadeIn relative z-10">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4">
                        <Link to={`/ListaAlumnos/${alumnoData.id_grupo}`} className="flex items-center text-[#8C1F2F] hover:text-[#8C1F2F] transition font-medium w-fit">
                            <ArrowLeft className="mr-2" /> Volver
                        </Link>
                        
                        {/* Botones de acción principales en Grid para móviles */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:w-auto">
                            <Link to={`/EventoCalendario`} state={{ alumno: alumnoData }} className="flex justify-center items-center bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-4 py-2 rounded-xl font-semibold shadow-md transition-transform hover:scale-[1.02] text-sm text-center">
                                <FaPlus className="mr-2" /> Registrar evento
                            </Link>
                            <Link to={`/Reportes`} state={{ alumno: alumnoData }} className="flex justify-center items-center bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-4 py-2 rounded-xl font-semibold shadow-md transition-transform hover:scale-[1.02] text-sm text-center">
                    ...
                    <h2 className="text-3xl font-bold text-gray-800 mb-2">Ficha del Alumno: {alumnoData.nombre_completo}.</h2>
    ...
    {/* --- SECCIÓN CALIFICACIONES (RESPONSIVE) --- */}
                    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-200 mb-8">
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full md:w-auto">
                                <button onClick={() => setAsignarCalifOpen(true)} className="bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-3 py-2 rounded-lg font-semibold shadow-sm text-sm transition text-center">
                                    + Calificación
                                </button>
                                <button onClick={() => setAsignarMateriaOpen(true)} className="bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-3 py-2 rounded-lg font-semibold shadow-sm text-sm transition text-center">
                                    + Asignar Mat.
                                </button>
                                <button onClick={() => setCrearMateriaOpen(true)} className="bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-3 py-2 rounded-lg font-semibold shadow-sm text-sm transition text-center">
                                    Crear Nueva
                                </button>
                            </div>
                        </div>
                        ...
                        {materiasCursadasConDetalle.length === 0 ? (
                            <div className="text-center py-8">
                                <img src={sinRegistrosImgUrl} alt="Sin calificaciones" className="mx-auto mb-4 w-48 rounded" />
                                <p className="text-gray-600">No hay materias o calificaciones asignadas aún.</p>
                            </div>
                        ) : (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {materiasCursadasConDetalle.map((mat, idx) => {
                                    ...
                                    const unidadesKeys = Object.keys(mat).filter(k => k.startsWith('unidad_')).sort((a, b) => {
                                        const na = Number(a.split('_')[1]), nb = Number(b.split('_')[1]);
                                        return na - nb;
                                    });
                                    // data con un solo elemento para mostrar barras por unidad para esa materia
                                    const data = [mat];
                                    return (
                                        <div key={idx} className="bg-gray-50 p-3 rounded-lg border-[#E9DBCD] border-2 shadow-sm">
                                            {/* Título truncado para que quepa en el grid */}
                                            <h4 className="font-semibold mb-2 text-center text-sm truncate" title={mat.nombre}>
                                                {mat.nombre}
                                            </h4>
                                            ...
                                            <div style={{ width: '100%', height: 200 }}>
                                                <ResponsiveContainer>
                                                    <BarChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                                                        ...
                                                        <YAxis domain={[0, 100]} allowDecimals={false} fontSize={12} />
                                                        <Tooltip />
                                                        {/* Leyenda opcional, si quieres más espacio la puedes comentar */}
                                                        {/* <Legend iconSize={8} /> */}
                                                        {unidadesKeys.map((uk, i) => (
                                                            <Bar key={uk} dataKey={uk} name={`U${uk.split('_')[1]}`} fill={["#8C1F2F", "#f97316", "#3b82f6", "#22c55e", "#8b5cf6"][i % 5]} />
                                                        ))}
                                                    </BarChart>
                                                </ResponsiveContainer>
                                            </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="lg:w-3/5 flex flex-col gap-6">
                            ...
                            <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                ...
                                <div className="space-y-1 text-sm md:text-base">
                                    ...
                                    <p><span className="font-semibold">Carrera:</span> {alumnoData.carrera || "No especificada"}</p>
                                ...
                                <div className="flex-grow space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                                    {bitacoraData.length > 0 ? (
                                        ...
                                        <div className="flex flex-col items-center py-8 text-center">
                                            <p className="text-gray-500 font-medium">No hay registros de bitácora.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="lg:w-2/5 flex flex-col gap-6">
                            ...
                            <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                <h3 className="text-lg md:text-xl font-bold mb-3">Materias inscritas</h3>
                                {cursadas.length === 0 ? (
                                    <p className="text-gray-500 text-sm">Sin materias asignadas.</p>
                                ) : (
                                    <div className="space-y-2 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
                                        {cursadas.map((c) => {
                                            const mat = materias.find(m => Number(m.id) === Number(c.id_materia));
                                            return (
                                                <div key={`${c.id_materia}-${c.id_alumno}`} className="p-3 rounded-lg flex justify-between items-center bg-gray-50 border-[#E9DBCD] border shadow-sm">
                                                    <div className="min-w-0 flex-1 mr-2">
                                                        <div className="font-semibold text-sm truncate" title={mat ? mat.nombre : ''}>
                                                            {mat ? mat.nombre : `Materia ${c.id_materia}`}
                                                        </div>
                                                        <div className="text-xs text-gray-500">Curso: {c.curso || "—"}</div>
                                                    </div>
                                                    
                                                    <div className="flex gap-1 items-center flex-shrink-0">
                                                        <button 
                                                            onClick={() => { setMateriaAEditar(c.id_materia); setEditarCalifOpen(true); }}
                                                        ...
                                                        <button 
                                                            onClick={() => { setMateriaParaCalif(c.id_materia); setAsignarCalifOpen(true); }} 
                        ...
                        {/* Disponibilidad */}
                            <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-200">
                                <h3 className="text-lg md:text-xl font-bold text-gray-800 mb-4">Disponibilidad</h3>

                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mb-3">
                                    <label className="text-sm text-gray-700 whitespace-nowrap">Ver día:</label>
                                    <select
                                        value={filtroDia}
                                        onChange={async (e) => {
                                            const d = e.target.value;
                                            setFiltroDia(d);
                                            d ? await fetchDisponibilidadesPorDia(alumnoData.matricula, d) : await fetchDisponibilidadesPorMatricula(alumnoData.matricula);
                                        }}
                                ...
                                {/* Formulario agregar */}
                                {mostrarFormulario && (
                                    <form onSubmit={handleAgregarDisponibilidad} className="mb-6 p-4  rounded-lg bg-gray-50 border-[#E9DBCD] shadow-sm border-2">
                                        <div className="mb-3">
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
                            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
                                ...
                                <form onSubmit={handleCrearMateria} className="space-y-3">
                                    <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" placeholder="Clave" value={nClave} onChange={(e) => setNClave(e.target.value)} />
                                    <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" placeholder="Nombre" value={nNombre} onChange={(e) => setNNombre(e.target.value)} />
                                    <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" placeholder="Grupo" value={nGrupo} onChange={(e) => setNGrupo(e.target.value)} />
                                    <div className="flex gap-2 justify-end">
                                        <button type="button" onClick={() => setCrearMateriaOpen(false)} className="px-4 py-2 rounded bg-gray-300 transition-transform hover:scale-[1.03]">Cancelar</button>
                                        <button type="submit" className="px-4 py-2 rounded bg-[#E4CD87] hover:bg-[#E9DBCD] transition-transform hover:scale-[1.03]">Crear</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    {asignarMateriaOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
                            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
                                <h4 className="text-lg font-bold mb-3">Asignar materia al alumno</h4>
                                <form onSubmit={handleAsignarMateria} className="space-y-3">
                                    <select className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" value={materiaSeleccionadaParaAsignar || ""} onChange={(e) => setMateriaSeleccionadaParaAsignar(e.target.value)}>
                                    ...
                                    <div className="flex gap-2 justify-end">
                                        <button type="button" onClick={() => setAsignarMateriaOpen(false)} className="px-4 py-2 rounded bg-gray-300 transition-transform hover:scale-[1.03]">Cancelar</button>
                                        <button type="submit" onClick={() => console.log("Click en boton asignar")} className="px-4 py-2 rounded bg-[#E4CD87] hover:bg-[#E9DBCD] transition-transform hover:scale-[1.03]">Asignar</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    {asignarCalifOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
                            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
                                ...
                                <form onSubmit={handleAsignarCalificacion} className="space-y-3">
                                    <select className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" value={materiaParaCalif || ""} onChange={(e) => setMateriaParaCalif(e.target.value)}>
                                        <option value="">Selecciona materia</option>
                                        {cursadas.map(c => {
                                            const m = materias.find(mm => Number(mm.id) === Number(c.id_materia));
                                            return <option key={c.id_materia} value={c.id_materia}>{m ? m.nombre : `Materia ${c.id_materia}`}</option>;
                                        })}
                                    </select>

                                    <div className="flex gap-2">
                                        <input type="number" min="1" className="w-1/3 px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" value={unidadCalif} onChange={(e) => setUnidadCalif(e.target.value)} />
                                        <input type="number" min="0" max="100" step="0.01" className="flex-1 px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E9DBCD] border-gray-300" placeholder="Calificación (0-100)" value={valorCalif} onChange={(e) => setValorCalif(e.target.value)} />
                                    </div>

                                    <div className="flex gap-2 justify-end">
                                        <button type="button" onClick={() => setAsignarCalifOpen(false)} className="px-4 py-2 rounded bg-gray-300 transition-transform hover:scale-[1.03]">Cancelar</button>
                                        <button type="submit" onClick={() => console.log("Click en boton guardar")} className="px-4 py-2 rounded bg-[#E4CD87] hover:bg-[#E9DBCD] transition-transform hover:scale-[1.03]">Guardar</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        {editarCalifOpen && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
                                <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">Ingresa la unidad que deseas corregir y la nueva nota.</p>
                                    
                                    <form onSubmit={handleActualizarCalificacion} className="space-y-3">
                                        ...
                                        <div className="p-2 bg-gray-100 rounded text-gray-700 font-medium">
                                            Materia: {materias.find(m => Number(m.id) === Number(materiaAEditar))?.nombre || "Cargando..."} 
                                            {/* Si quieres mostrar el nombre, busca la materia en el array 'materias' usando el ID */}
                                        </div>

                                        <div className="flex gap-2">
                                            <div className="w-1/3">
                                                <label className="text-xs font-bold text-gray-00">Unidad</label>
                                                <input 
                                                    type="number" 
                                                    placeholder="Ej. 1"
                                            ...
                                            <div className="flex-1">
                                                <label className="text-xs font-bold text-gray-500">Nueva Calificación</label>
                                                <input 
                                                    type="number" 
                                                    step="0.01" 
                                                    placeholder="0-100"
                                        ...
                                        <div className="flex gap-2 justify-end mt-4">
                                            <button type="button" onClick={() => setEditarCalifOpen(false)} className="px-4 py-2 rounded bg-gray-300 transition-transform hover:scale-[1.03] font-medium">Cancelar</button>
                                            <button type="submit" className="px-4 py-2 rounded bg-[#E4CD87] hover:bg-[#E9DBCD] transition-transform hover:scale-[1.03] font-bold">Actualizar</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
