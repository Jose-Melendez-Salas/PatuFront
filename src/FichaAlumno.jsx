    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setBitacoraData(
                    (bitacoraJson.data || [])
                        .filter(item => item !== null && item !== undefined) // ✅ elimina nulos
                        .map(item => ({
                            ...item,
            ...
            <main className="p-4 animate-fadeIn relative z-10">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <Link to={`/ListaAlumnos/${alumnoData.id_grupo}`} className="flex items-center text-[#8C1F2F] hover:text-[#8C1F2F] transition font-medium">
                            <ArrowLeft className="mr-2" /> Volver
                        </Link>
                        <div className="flex items-center gap-4">
                            <button onClick={() => navigate("/EventoCalendario")} className="flex items-center justify-center gap-2 bg-[#E4CD87] text-black font-bold px-5 py-2 rounded-full shadow-md hover:bg-[#E9DBCD] transition-transform hover:scale-[1.03]">
                                <FaPlus className="text-sm" /> Registrar evento
                            </button>
                            <Link to={`/Reportes`} state={{ alumno: alumnoData }} className="flex items-center bg-[#E4CD87] hover:bg-[#E9DBCD] text-black px-5 py-2 rounded-full font-semibold shadow-md transition-transform hover:scale-[1.03]">
                                    ...
                                    {bitacoraData.length > 0 ? (
                                        ...
                                        .map((item, index) => (
                                            <BitacoraFicha
                                                ...
                                                asistencia={item.asistencia || "Sin registro"}
                                                notas={item.notas || ""}
                                                acuerdos={item.acuerdos || ""}
                                                compromisos={item.compromisos || ""}
                                                color="#4F3E9B"
                                    ...
                                    <form onSubmit={handleAgregarDisponibilidad} className="mb-6 p-4 border rounded-lg bg-gray-50">
                                        ...
                                        <div className="flex gap-3">
                                            <button
                                                ...
                                                className="flex-1 bg-[#E4CD87] hover:bg-[#E9DBCD] text-black font-bold py-2 px-4 rounded-md transition disabled:opacity-50"
                                            >
