        ...
        <form className="flex flex-col gap-4 sm:gap-5" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col relative">
            ...
            <input
              ...
              className="p-2 sm:p-3 border border-gray-300 rounded-2xl pr-10 sm:pr-12 focus:outline-none focus:ring-2 focus:ring-purple-400 w-full"
              disabled={cargando}
            />
            <button
              type="button"
              onClick={() => setMostrarPassword(!mostrarPassword)}
              className="absolute right-3 sm:right-4 top-10 text-gray-500 hover:text-gray-700"
              disabled={cargando}
            >
          ...
          {/* Mensajes */}
          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 sm:px-4 sm:py-3 rounded-xl text-sm text-center">
              {error}
            </div>
          )}
          {mensaje && !redirigiendo && (
            <div className="bg-green-50 border border-green-200 text-green-700 px-3 py-2 sm:px-4 sm:py-3 rounded-xl text-sm text-center">
              {mensaje}
            </div>
          )}
          {redirigiendo && (
            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2 sm:px-4 sm:py-3 rounded-xl text-sm text-center flex items-center justify-center gap-3">
          ...
          {cargando && (
            <div className="flex items-center justify-center gap-3 py-3">
              <div className="w-5 h-5 sm:w-6 sm:h-6 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin"></div>
              <span className="text-gray-600 font-medium text-sm sm:text-base">Actualizando contraseña...</span>
            </div>
          )}
          ...
          <button
            type="submit"
            className="bg-[#3CB9A5] hover:bg-[#1f6b5e] text-white py-2 sm:py-3 px-4 sm:px-6 rounded-2xl font-bold text-base sm:text-lg mt-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
