...
const MensajesCordi = () => {
    ...
    useEffect(() => {
        const obtenerMensajes = async () => {
            try {
                ...
                setModal({ abierto: true, tipo: 'error', mensaje: error.message });
            } finally {
                setCargando(false);
            }
        };
    ...
    const handleMarcarRecibido = async (id) => {
        try {
            ...
            setModal({ abierto: true, tipo: 'success', mensaje: '✅ Mensaje marcado como recibido.' });
        } catch (error) {
            setModal({ abierto: true, tipo: 'error', mensaje: error.message });
        }
    };
        ...
        setModal({
            ...
            onConfirm: async () => {
                try {
                    ...
                    setModal({ abierto: true, tipo: 'success', mensaje: '🗑️ Mensaje eliminado correctamente.' });
                } catch (error) {
                    setModal({ abierto: true, tipo: 'error', mensaje: error.message });
                }
            }
